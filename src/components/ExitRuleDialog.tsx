...
import { ExitReason } from '@/types/recommendation';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
interface ExitRuleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (exitReason: ExitReason, exitPrice?: number) => void;
  stockName: string;
  target1: number;
  target2: number;
  target3: number;
...
const exitOptions: { value: ExitReason; label: string; requiresPrice?: boolean; isLoss?: boolean }[] = [
...
export function ExitRuleDialog({
  ...
  const [selectedReason, setSelectedReason] = useState<ExitReason>(null);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="bg-card border-border max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select exit reason to close this recommendation
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2 text-center text-xs">
            ...
            <div className="p-2 bg-muted rounded-lg">
              <p className="text-muted-foreground">T3</p>
              <p className="font-mono font-medium text-profit">₹{target3}</p>
            </div>
            <div className="p-2 bg-muted rounded-lg">
